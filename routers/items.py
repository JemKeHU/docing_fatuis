...
from schemas.items import ItemBase, ItemCreate, ItemChange
...
@item_router.get("")
async def get_items(session: SessionDep, limit: int = 10, offset: int = 0, keyword: str | None = None):
    filtered_items = []
    if keyword:
        for item in items:
...
@item_router.post("")
async def post_item(item: ItemBase, session: SessionDep) -> ItemCreate:
    global UNIQUE_ID
...
async def change_item_completely(item_id: int, new_item: ItemChange, session: SessionDep):
    for item in items:
        if item.id == item_id:
...
async def change_done(item_id: int, is_done: bool, session: SessionDep):
    for item in items:
        if item.id == item_id:
